<x-admin-layout>
    <div class="max-w-4xl mx-auto space-y-8">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-serif font-bold text-foreground">Edit Artikel</h1>
                <p class="text-muted-foreground font-medium">Perbarui konten artikel semantik dan metadata.</p>
            </div>
        </div>
            ...
            <form action="{{ route('news.update', $news->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        <label for="title" class="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Judul Artikel</label>
                    ...
                    <div>
                        <label for="category" class="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Kategori Semantik</label>
                    ...
                    <div>
                        <label for="source" class="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Sumber / Penulis</label>
                    ...
                    <div>
                        <label for="published_at" class="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Tanggal Publikasi</label>
                    ...
                    <div class="md:col-span-2 border-t border-border pt-6">
                        <label class="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-4">Gambar Poster</label>
                        
                        @if($news->image)
                        <div class="mb-4">
                            <img src="{{ $news->image }}" alt="Current Poster" class="w-32 h-32 object-cover rounded-xl border border-border shadow-sm">
                            <p class="text-[10px] text-muted-foreground mt-2 italic">Gambar Saat Ini: {{ $news->image }}</p>
                        </div>
                        @endif

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label for="image_file" class="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Unggah File Baru</label>
                            ...
                            <div>
                                <label for="image_url" class="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Atau Gunakan URL Gambar</label>
                ...
                <div>
                    <label for="content" class="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Isi Artikel</label>
                ...
                <div class="pt-4">
                    <button type="submit" class="w-full bg-primary text-primary-foreground py-4 rounded-2xl font-bold shadow-lg shadow-primary/20 hover:scale-[1.02] transition-all">
                        PERBARUI ARTIKEL & SINKRON TRIPLE
                    </button>
                </div>
            </form>
        </div>
